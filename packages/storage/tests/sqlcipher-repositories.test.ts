...
describe('sqlcipher repositories', () => {
  it('stores keys in keychain or keystore and verifies encrypted at rest', async () => {
    ...
    expect(iosConnection.getEncryptionInfo().keyStorePlatform).toBe('ios-keychain');
    expect(iosConnection.getEncryptionInfo().keyAlias).toBe('marginbase.mobile.db');
    ...
    expect(androidConnection.getEncryptionInfo().keyStorePlatform).toBe('android-keystore');
  });
  ...
  it('executes scenario, settings, entitlements and schema operations after encryption integration', async () => {
    ...
    const entitlementRepository = new SqlCipherEntitlementRepository(connection);
    const schemaStateRepository = new SqlCipherSchemaStateRepository(connection);

    await scenarioRepository.upsertScenario(sampleScenario);
    expect((await scenarioRepository.listScenarios()).length).toBe(1);

    await settingsRepository.setSetting({ key: 'theme', value: 'dark', updatedAt: '2026-03-02T10:00:00.000Z' });
    ...
    expect((await entitlementRepository.getEntitlementCache('user_sqlcipher_1'))?.entitlementSet.profit).toBe(true);
  ...
  it('supports migration strategy wipe from plaintext sqlite', async () => {
    ...
    const sourceScenarioRepository = new SqlitePlaceholderScenarioRepository(source);
    ...
    const encryptedScenarioRepository = new SqlCipherScenarioRepository(encrypted);
  ...
  it('supports migration strategy migrate from plaintext sqlite', async () => {
    const source = new SqlitePlaceholderConnection();
    const sourceScenarioRepository = new SqlitePlaceholderScenarioRepository(source);
    const sourceSettingsRepository = new SqlitePlaceholderSettingsRepository(source);

    await sourceScenarioRepository.upsertScenario(sampleScenario);
    await sourceSettingsRepository.setSetting({ key: 'numberFormat', value: 'eu', updatedAt: '2026-03-02T10:00:00.000Z' });
    ...
    const encryptedScenarioRepository = new SqlCipherScenarioRepository(encrypted);
    const encryptedSettingsRepository = new SqlCipherSettingsRepository(encrypted);
    const encryptedSchemaStateRepository = new SqlCipherSchemaStateRepository(encrypted);

    expect((await encryptedScenarioRepository.listScenarios()).length).toBe(1);
    expect((await encryptedSettingsRepository.getSetting('numberFormat'))?.value).toBe('eu');
