...
describe('sqlite placeholder repositories', () => {
  it('executes scenario CRUD and replace-all via repository', async () => {
    ...
    await repository.replaceAllScenarios([
      { ...sampleScenario, scenarioId: 'scenario_sqlite_2', module: 'profit', scenarioName: 'Replaced scenario' }
    ]);
  ...
  it('stores settings and entitlement cache via repositories', async () => {
    ...
    await settings.setSetting({ key: 'locale', value: 'en-US', updatedAt: '2026-03-02T10:00:00.000Z' });
    ...
    await entitlements.setEntitlementCache({
      ...
      entitlementSet: { bundle: false, profit: true, breakeven: true, cashflow: false }
    });
    expect((await entitlements.getEntitlementCache('user_sqlite_1'))?.entitlementSet.breakeven).toBe(true);
    expect(await entitlements.clearEntitlementCache('user_sqlite_1')).toBe(true);
  });
